...
function Fire({ color, ...props }: { color?: THREE.Color } & React.ComponentProps<'mesh'>) {
  const ref = useRef<THREE.Mesh>(null!)
  const texture = useLoader(THREE.TextureLoader, fireImg)
  useFrame((state) => {
    if (!ref.current) return
    const mat = ref.current.material as THREE.ShaderMaterial
    const inv = mat.uniforms.invModelMatrix.value as THREE.Matrix4
    ref.current.updateMatrixWorld()
    inv.copy(ref.current.matrixWorld).invert()
    mat.uniforms.time.value = state.clock.elapsedTime
    mat.uniforms.invModelMatrix.value = inv
  ...
  return (
    <mesh ref={ref} {...props}>
      <boxGeometry />
      {/* @ts-expect-error - fireMaterial extended at runtime */}
