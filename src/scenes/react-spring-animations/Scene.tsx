import { useState, useEffect, useCallback, Suspense, type Dispatch, type SetStateAction } from 'react'
...
function SwitchModel({ x, set }: { x: X; set: Dispatch<SetStateAction<number>> }) {
  const { nodes, materials } = useGLTF(switchModel) as unknown as {
...
export function Scene({ x, set }: { x: X; set: Dispatch<SetStateAction<number>> }) {
  ...
  return (
    <Canvas orthographic shadows dpr={[1, 2]} camera={{ zoom: 60, position: [-10, 10, 10], fov: 35 }}>
      ...
      <Suspense fallback={null}>
        <SwitchModel x={x} set={set} />
      </Suspense>
      <mesh receiveShadow renderOrder={1000} position={[0, 0, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[10, 10]} />
        <a.shadowMaterial transparent opacity={x.to((v: number) => 0.1 + v * 0.2)} />
      </mesh>
    </Canvas>
  )
}
